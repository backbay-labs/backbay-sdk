...
function createFileBrowserStoreImpl(set: any, get: any): FileBrowserStore {
  return {
    ...
    navigateUp: (files) => {
      ...
      const currentFolder = files.find((f: FileItem) => f.id === state.currentFolderId);
      if (!currentFolder) return;
      get().navigateTo(currentFolder.parentId);
    },

    select: (id, additive) => {
      set((state: FileBrowserStore) => {
...
/** Factory: creates an isolated FileBrowser store instance. */
export function createFileBrowserStore() {
  return createStore<FileBrowserStore>((set, get) => createFileBrowserStoreImpl(set, get));
}

export type FileBrowserStoreApi = ReturnType<typeof createFileBrowserStore>;

const FileBrowserStoreContext = createContext<FileBrowserStoreApi | null>(null);

/** Provider that creates an isolated FileBrowser store for its subtree. */
export function FileBrowserStoreProvider({ children }: { children: ReactNode }) {
  const [store] = useState(() => createFileBrowserStore());
  return (
