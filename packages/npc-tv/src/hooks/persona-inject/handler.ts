...
function buildPersonaPrompt(config: NpcTvConfig): string {
  ...
  if (!config.features.chat) {
    templateText = templateText
      .replace(/\s*When you see chat messages via npc_read_chat,\s*acknowledge interesting ones\./g, '')
...
function wrapPersonaDoc(corePrompt: string, config: NpcTvConfig): string {
  ...
  if (config.features.reactions) {
    sections.push('- `npc_react` — Send a reaction/emote to the stream (celebration, thinking, frustrated, mind_blown, ship_it)');
  }
  ...
  if (config.features.chat) {
    sections.push('- Periodically use `npc_read_chat` to check for viewer messages and respond to interesting ones.');
    sections.push('- Chat messages may arrive at any time. Check frequently so viewers feel heard.');
  }
...
const handler: HookHandler = async (event: HookEvent): Promise<void> => {
  ...
  const shouldInject = pluginConfig.channel.autoGoLive ||
    (channelManager !== null && channelManager.isLive());
