...
describe("Relay HTTP Routes", () => {
  ...
  test("GET /channels/:id returns channel details", async () => {
    ...
    const channelId = (
      (createRes.json as Record<string, unknown>).data as Record<
        string,
        unknown
      >
    ).id as string;
    ...
    expect(status).toBe(200);
    expect(
      ((json as Record<string, unknown>).data as Record<string, unknown>).name,
    ).toBe("Details Test");
  });
  ...
  test("DELETE /channels/:id deregisters with correct API key", async () => {
    ...
    const data = (createRes.json as Record<string, unknown>).data as Record<
      string,
      unknown
    >;
  ...
  test("POST /channels/:id/chat sends a message", async () => {
    ...
    const { status, json } = await req(
      "POST",
      `/channels/${data.id}/chat`,
      { body: { author: "viewer1", content: "Hello!" } },
    );

    expect(status).toBe(200);
    const msgData = (json as Record<string, unknown>).data as Record<
      string,
      unknown
    >;
  ...
  test("GET /channels/:id/stream returns SSE with correct headers", async () => {
    ...
    const res = await app.handle(
      new Request(`http://localhost/channels/${channelId}/stream`),
    );
