...
function createSystemTrayStoreImpl(set: any): SystemTrayStore {
  return {
    ...
    toggleExpanded: () => {
      set((state: SystemTrayStore) => ({ isExpanded: !state.isExpanded }));
    },
  };
}

/** Factory: creates an isolated SystemTray store instance. */
export function createSystemTrayStore() {
  return createStore<SystemTrayStore>((set) => createSystemTrayStoreImpl(set));
}

export type SystemTrayStoreApi = ReturnType<typeof createSystemTrayStore>;

const SystemTrayStoreContext = createContext<SystemTrayStoreApi | null>(null);

/** Provider that creates an isolated SystemTray store for its subtree. */
export function SystemTrayStoreProvider({ children }: { children: ReactNode }) {
  const [store] = useState(() => createSystemTrayStore());
  return (
    <SystemTrayStoreContext.Provider value={store}>
      {children}
    </SystemTrayStoreContext.Provider>
  );
}
